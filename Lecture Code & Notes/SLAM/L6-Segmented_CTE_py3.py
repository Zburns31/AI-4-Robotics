...
class plan:
    ...
    def astar(self):
        ...
        delta = [[-1, 0], [0, -1], [1, 0], [0, 1]]  # go up  # go left  # go down  # do right
...
class robot:
    ...
    def measurement_prob(self, measurement):
        ...
        # calculate Gaussian
        error = exp(-(error_x ** 2) / (self.measurement_noise ** 2) / 2.0) / sqrt(
            2.0 * pi * (self.measurement_noise ** 2)
        )
        error *= exp(-(error_y ** 2) / (self.measurement_noise ** 2) / 2.0) / sqrt(
            2.0 * pi * (self.measurement_noise ** 2)
        )
...
class particles:
    ...
    def get_position(self):
        ...
        for i in range(self.N):
            ...
            # the 0=2pi problem
            orientation += (
                ((self.data[i].orientation - self.data[0].orientation + pi) % (2.0 * pi))
...
def run(grid, goal, spath, params, printflag=False, speed=0.1, timeout=1000):
    ...
    filter_ = particles(
        myrobot.x, myrobot.y, myrobot.orientation, steering_noise, distance_noise, measurement_noise
    )
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        # U represents the robot estimate projected onto the path segment
