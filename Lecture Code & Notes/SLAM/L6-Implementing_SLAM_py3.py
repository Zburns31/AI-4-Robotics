...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    # Add your code here!
    #
    # Length of the path + number of landmarks * 2 because we have x and y
    dim = 2 * (N + num_landmarks)

    # Make the constraint matrix and vector
    Omega = matrix()
    Omega.zero(dim, dim)

    # Strength values for the constraints
    Omega.value[0][0] = 1.0  # initial constraint X
    Omega.value[1][1] = 1.0  # initial constraint Y

    Xi = matrix()
    Xi.zero(dim, 1)
    Xi.value[0][0] = world_size / 2.0  # X
    Xi.value[1][0] = world_size / 2.0  # Y
    ...
    for k in range(len(data)):
        ...
        for i in range(len(measurement)):
            ...
            # m in the index of the landmark coordinate in the matrix/vector
            # * 2 because of X/Y again
            m = 2 * (N + measurement[i][0])

            # update the info matrxi/vector based on the measurement
            for b in range(2):
        ...
        # update the info matrix/vector based on the motion
        for b in range(4):
...
# sequence of motions and measurements
data = make_data(
    N, num_landmarks, world_size, measurement_range, motion_noise, measurement_noise, distance
)
...
test_data2 = [
    [
        [[0, 26.543274387283322, -6.262538160312672], [3, 9.937396825799755, -9.128540360867689]],
